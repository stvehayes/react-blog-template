    ...
    switch (node.kind) {
        ...
        case ts.SyntaxKind.ElementAccessExpression:
            return hasSideEffects(node.expression, options) ||
                ...
                node.argumentExpression !== undefined && // for compatibility with typescript@<2.9.0
        ...
        case ts.SyntaxKind.NewExpression:
            if (options & 2 /* Constructor */ || hasSideEffects(node.expression, options))
                return true;
        ...
        case ts.SyntaxKind.TaggedTemplateExpression:
            if (options & 1 /* TaggedTemplate */ || hasSideEffects(node.tag, options))
                return true;
...
function classExpressionHasSideEffects(node, options) {
    ...
    for (const child of node.members)
        if (!hasModifier(child.modifiers, ts.SyntaxKind.DeclareKeyword) && (child.name !== undefined && child.name.kind === ts.SyntaxKind.ComputedPropertyName &&
            hasSideEffects(child.name.expression, options) ||
...
class ImportFinder {
    ...
    _findNestedImports() {
        ...
        else if (this._options & 16 /* Require */) {
